...
Приложение попадает в стек, если оно было установлено из него (`asc install
<стек>` или `asc install <стек>/<приложение>`); приложение, установленное
само по себе, не принадлежит ни одному стеку и сюда не попадает — полный
список приложений независимо от происхождения — [`asc ls`](/ru/commands/ls).
